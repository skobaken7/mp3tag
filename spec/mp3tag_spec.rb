...
describe Mp3tag do
  describe "#expand_param_files" do
    it 'eliminates files other than mp3' do
      ...
      result = described_class.expand_param_files(["aaa.mp3", "bbb.txt", "ccc.mp3", "d.jpg"])
      expect(result).to eq(["aaa.mp3", "ccc.mp3"])
    end

    it 'raises exception when files are not found' do
      expect {
        described_class.expand_param_files(["aaa.mp3"])
    ...
    it 'includes all files under the dirs' do
      ...
      result = described_class.expand_param_files(["aaa.mp3", "bbb.txt", "ccc.mp3", "a"])
